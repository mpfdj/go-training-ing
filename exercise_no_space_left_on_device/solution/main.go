...
func printTree(node *Node, padding int) {
	print(node.Name)
	print("    Size:")
	print(node.Size)
	print("\n")
	...
	for index, v := range node.Children {
		padding++
...
func nodeSize(node *Node) {
	if len(node.Children) > 0 {
		for _, v := range node.Children {
			nodeSize(v)
		}
	} else {
		addSize(node, node.Size)
	}
}
...
func sumSmallFolders(node *Node) int {
	totalSum := 0
	if len(node.Children) > 0 {
		if node.Size <= 100000 {
			totalSum += node.Size
...
202817 dvtblw.wsr`
	...
	for _, line := range strings.Split(strings.TrimSuffix(input, "\n"), "\n") {
		...
		if result[0] == "$" {
			...
			tmp := &Node{Name: result[1], Parent: current}
	...
	print("Sum small folders")
	print(sumSmallFolders(root))
}
